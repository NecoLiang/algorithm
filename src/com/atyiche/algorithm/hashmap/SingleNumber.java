...
public class SingleNumber {
    ...
    public int singleNumber2(int[] nums){
        HashMap<Integer, Integer> singlerNumMap = new HashMap<>();
        for (Integer num : nums) {
            if (singlerNumMap.get(num) != null){
                singlerNumMap.remove(num);
            }else {
                singlerNumMap.put(num,1);
            }
        }
        return singlerNumMap.keySet().iterator().next();
    }

    //方法三：用set方法去重，a = 2 * (a + b + c) - (a + b + c + b + c)
